//strict mode
//ES5 버전의 기능. 엄격한 운용 콘텍스트 안에서 실행할 수 있게끔 함.
//에러를 좀 더 엄격하게 잡는 다는 의미.
'use strict';
...
app.controller('DashController', function ($scope, sources, contributors, dashifTestVectors) {
    ...
    $scope.player.on(dashjs.MediaPlayer.events.PERIOD_SWITCH_COMPLETED, function (e) { /* jshint ignore:line */
        ...
        console.log('[DEBUG] [dash Player] streamInfo');
        console.log(e.toStreamInfo);
    }, $scope);
    ...
    $scope.player.on(dashjs.MediaPlayer.events.PLAYBACK_ENDED, function (e) { /* jshint ignore:line */
        if ($('#loop-cb').is(':checked') &&
            ...
            //TEST! 스트림이 끝났을 때 발생하는..!
            console.log('[DEBUG] ' +  $scope.player.getActiveStream().getStreamInfo().isLast);
            console.log($scope.player.getActiveStream().getStreamInfo());
            $scope.doLoad(); //해당 스트림이 마지막 스트림이면 Load 버튼을 누르게함. mpd를 다시 받는 것.
        }
    }, $scope);

    $scope.player.on(dashjs.MediaPlayer.events.FRAGMENT_LOADING_COMPLETED, (e) => {
        var request = e.request;
        console.log('[DEBUG] [dash player] fragment loading');
        console.log(e);
        console.log(request.mediaInfo.streamInfo);
        console.log(e.sender.getStreamProcessor());
        console.log(e.sender.getStreamProcessor().getFragmentController());
        console.log(e.sender.getStreamProcessor().getRepresentationController());
        console.log(e.sender.getStreamProcessor().getRepresentationController().getData());
        console.log(e.sender.getStreamProcessor().getRepresentationController().getDataIndex());
        console.log($scope.player);
        console.log(dashjs);
    }, $scope);

    $scope.player.on(dashjs.MediaPlayer.events.USER_INBANDEVENT_RECEIVED, (e) => {
        console.log('[DEBUG] [dash Player] USER_INBANDEVENT_RECEIVED');
        console.log(e);
        eventInfo = e;
        event_flag = e.value;
        event_start_time = e.event.presentationTime - e.event.presentationTimeDelta;
        console.log('[TEST][INBAND_EVENT_HANDLE][PLAYER] USER_INBANDEVENT_RECEIVED' + '\n\tEvent type: ' + e.type + '\n\tEvent Scheme: ' + e.event.eventStream.schemeIdUri + '\n\tEvent value: ' + e.value + '\n\tEvent StartTime: ' + (e.event.presentationTime - e.event.presentationTimeDelta))  ;
    })
    $scope.player.on(dashjs.MediaPlayer.events.PLAYBACK_TIME_UPDATED, (e) => {
        console.log('[DEBUG] [dash Player] PLAYBACK_TIME_UPDATED');
        console.log(e);
        var url;
        if(event_flag != 0){
            if(e.time > event_start_time){
                if(event_flag == 1){
                    url = 'http://localhost/LIVE_1/dash.mpd';
                } else if (event_flag == 2){
                    url = 'http://localhost/LIVE_2/dash.mpd';
                }
                console.log('[TEST][INBAND_EVENT_HANDLE][PLAYER] Event Handler(web) run' + '\n\tPresent Time: ' + e.time + '\n\tEvent StartTime: ' + event_start_time + '\n\tNew attach Source Target Uri: ' + url);
                $scope.player.attachSource(url);
                xhr.open('POST', 'http://localhost:3000/events');   
                xhr.setRequestHeader('Content-type', 'application/json');
                var data = {event: eventInfo, new_url: url};
    ...
    $scope.setStream = function (item) {
        ...
        console.log("[DEBUG] - selectedItem : " + JSON.parse(JSON.stringify(item)));
    };
    ...
    $scope.doLoad = function () {
        ...
        if ($scope.selectedItem.hasOwnProperty('protData')) {
            ...
            //proData가 무엇인가
            console.log('[DEBUG] - proData : ' + proData);
        ...
        //attachSource의 url을 변경하면 자동으로 실행 되는가?
        //$scope.player.attachSource('http://localhost/manifest_toy.mpd');
